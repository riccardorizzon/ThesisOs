...
class SpyTaskService(TaskService):
    def __init__(self):
        self.upserts: list[tuple[TaskRef, str | None, list[str]]] = []
    ...
    async def upsert_from_task_ref(
        ...
        *,
        ...
        self.upserts.append((task_ref, owner_agent, list(plan_steps)))
...
async def test_planner_calls_persist_hook_on_happy_path():
    ...
    upserts: list[tuple[TaskRef, list[str]]] = []

    async def on_task_ref(task: TaskRef, plan_steps: list[str]) -> None:
        upserts.append((task, list(plan_steps)))
...
async def test_planner_continues_when_persist_hook_raises():
    async def failing_hook(task: TaskRef, plan_steps: list[str]) -> None:
